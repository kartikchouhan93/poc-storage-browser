...
export async function PUT(
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const policiesToCreate = Object.entries(policies)
        .map(([bucketId, actions]) => ({
          teamId: teamId,
          resourceType: "Bucket",
          resourceId: bucketId,
    ...
    logAudit({
      userId: user.id,
      action: "TEAM_POLICIES_UPDATED",
      resource: "ResourcePolicy",
      resourceId: teamId,
      status: "SUCCESS",
      ipAddress: extractIpFromRequest(request),
      details: { teamId, teamName: team.name, policies },
    });
