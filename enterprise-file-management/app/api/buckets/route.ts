  ...
  return withTenantAccess(
    ...
    async (req, middlewareUser) => {
      try {
        ...
        // Resolve the effective user: bot token overrides middleware user
        ...
        if (botAuth) {
          ...
          // Re-fetch with full includes if middleware user lacks them
          ...
          if (isExisting) {
            ...
            try {
              ...
              if (!awsAccount) {
                ...
                if (!fileSyncQueueArn) {
                  console.warn(
                    "FILE_SYNC_QUEUE_ARN not set — skipping S3 notification setup for same-account bucket",
                  );
                } else {
                  try {
                    ...
                    console.log(
                      `S3 → SQS notification configured for same-account bucket: ${finalBucketName}`,
                    );
                  } catch (err) {
                    console.warn(
                      "Could not configure S3 notification for same-account bucket:",
                      err,
                    );
                  }
                }
              ...
              try {
                await s3.send(
                  new DeleteBucketCommand({ Bucket: finalBucketName }),
                );
              } catch (cleanupError) {
                console.error(
