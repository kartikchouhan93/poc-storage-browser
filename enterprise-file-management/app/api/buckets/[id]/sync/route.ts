  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "AWS credentials not found and AWS_PROFILE/AWS_ACCESS_KEY_ID are not configured",
        },
        { status: 400 },
      );
    }
    ...
    if (
      ...
      versioningRes.value.Status === "Enabled"
    ) {
      versioning = true;
    }
    ...
    // Process Encryption
    if (
      encryptionRes.status === "fulfilled" &&
    ...
    do {
      const objectsData: any = await s3.send(
    ...
    return NextResponse.json({
      bucket: {
        ...updatedBucket,
        quotaBytes: Number(updatedBucket.quotaBytes),
      },
    ...
    return NextResponse.json(
      { error: error.message || "Failed to sync bucket" },
      { status: 500 },
    );
  }
}
