...
export function FileBrowser({ onUploadClick }: FileBrowserProps) {
  ...
  const currentFiles = React.useMemo(() => {
    ...
    for (const segment of path) {
      const folder = files.find(
        (f) => f.type === "folder" && f.name === segment
      )
      if (folder?.children) {
          ...
          <Table>
            ...
            <TableBody>
              {currentFiles.map((file) => {
                ...
                const color = fileColors[file.type]
                return (
                  <TableRow
                    key={file.id}
            ...
            return (
              <Card
                ...
                className={`group cursor-pointer transition-colors hover:bg-accent/50 ${
                  selected.has(file.id) ? "ring-2 ring-primary" : ""
                }`}
                onClick={() =>
                  file.type === "folder"
                    ? navigateToFolder(file.name)
