...
async function upsertTeam(name: string, tenantId: string) {
  let team = await prisma.team.findFirst({ where: { name, tenantId, isDeleted: false } });
  if (!team) {
...
async function upsertTeamBucketPolicy(
  ...
  if (!existing) {
    ...
    console.log(`  ✅ Created team policy for bucket: ${bucketId} → [${actions.join(', ')}]`);
  } else {
    ...
    console.log(`  ⏭️  Updated team policy for bucket: ${bucketId} → [${actions.join(', ')}]`);
  }
}
